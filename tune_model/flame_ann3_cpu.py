...
def main(args):
    ...
    y_train = torch.tensor(df_train[["T"]].values, device='cpu')
    X_test = torch.tensor(df_test[["CO+CO2", "N2"]].values, device='cpu')
    y_test = torch.tensor(df_test[["T"]].values, device='cpu')

    loss, T_err = dl_modeling(X_train, y_train, X_test, y_test, args.nodes, n_epoch=args.epochs)
    node_str = '\t'.join([str(x) for x in args.nodes])
    print(f"{node_str}\t{loss:.4f}\t{T_err:.4f}")
