...
def main(args):
    ...
    y_train = torch.tensor(df_train[["n2_out"+ f"{v}" for v in range(1, 51)]].values, device='cpu')
    X_test = torch.tensor(df_test[["CO+CO2", "H*", "O*", "C*"]].values, device='cpu')
    y_test = torch.tensor(df_test[["n2_out"+ f"{v}" for v in range(1, 51)]].values, device='cpu')
