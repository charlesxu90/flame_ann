...
def main(args):
    ...
    df_train = pd.read_pickle(data_dir + "train1_norm_resampled.pkl")
    df_test = pd.read_pickle(data_dir + "train1_norm_resampled.pkl")


    X_train = torch.tensor(df_train[["CO+CO2", "N2"]].values, device='cpu')
    y_train = torch.tensor(df_train[["n2_out"+ f"{v}" for v in range(1, 50)]].values, device='cpu')
    X_test = torch.tensor(df_test[["CO+CO2", "N2"]].values, device='cpu')
    y_test = torch.tensor(df_test[["n2_out"+ f"{v}" for v in range(1, 50)]].values, device='cpu')

    loss, M1_err, M2_err = dl_modeling(X_train, y_train, X_test, y_test, args.nodes, n_epoch=args.epochs)
    node_str = '\t'.join([str(x) for x in args.nodes])
    print(f"{node_str}\t{loss:.4f}\t{M1_err:.4f}\t{M2_err:.4f}")
...
def parse_args():
    ...
    parser.add_argument("--epochs", type=int, default=100, help="Epochs in Tensorflow, default 100")
